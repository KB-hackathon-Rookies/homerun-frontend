...
- **체크리스트가 있는 화면은 다 체크해야 다음 CTA 가 눌린다.** 시안에 명시가 없어도 기본값이다 — 매번 물어보지 않는다. `computed` 로 `모두 체크됨` 을 만들어 버튼 `:disabled` 에 물린다. 해당 없는 항목(조건부로 빠지는 서류 등)은 애초에 체크 대상에서 제외한다. 예외: 현장에서 스스로 짚어 보라고 두는 관찰용 체크리스트(예: 임장 체크리스트, `visit.vue`)는 저장도 게이트도 하지 않는다 — 화면 주석에 그 의도가 적혀 있으면 그대로 둔다
...
| `brand/`  | 로고·마스코트 마크                                                                     |
| `tiger/`  | 호랑이 일러스트                                                                        |
| `icon/`   | 야구 픽토그램(공·글러브·배트·헬멧·야구장·코치·계산기). 코치 시트 머리와 카드 앞에 쓴다 |
| `illust/` | 그 밖의 그림·사진                                                                      |
