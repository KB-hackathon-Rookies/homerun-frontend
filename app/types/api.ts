 ...
 * 백엔드 공통 응답 봉투.
 *
 * 백엔드가 성공 응답을 이 형태로 감싼다(`global.response.ApiResponse`).
 * 실제 값은 `data` 안에 있다.
 *
 * **다만 전부는 아니다.** 주소 검색·건축물대장·실거래·월세 계산 여덟 곳은
 * DTO 를 그대로 내보낸다(백엔드 #323). 새 API 를 붙일 때 봉투가 있는지
 * 확인하고 쓸 것 — 틀려도 컴파일은 되고 화면에서만 빈칸으로 드러난다.
 *
