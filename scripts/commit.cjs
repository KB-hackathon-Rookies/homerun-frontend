...
const commitTypes = [
  '✨ Feat', '📦 Chore', '💄 Design', '🚨 Fix', '🎨 Style',
  '⚡ Perf', '🔥 Remove', '🚀 Release', '🎉 Init', '✅ Test',
  '🔒 Security', '♻ Refactor', '🔨 Modify', '🚚 Rename', '📝 Docs',
  '➖ Remove', '🔖 Release', '🧪 Test', '🚑 Hotfix'
];
...
async function selectCommitType(rl) {
  console.log('\n커밋 타입을 선택하세요.');
  commitTypes.forEach((commitType, index) => console.log(`  ${index + 1}. ${commitType}`));

  while (true) {
    const selectedIndex = Number.parseInt(await rl.question('\n번호: '), 10) - 1;
    if (Number.isInteger(selectedIndex) && commitTypes[selectedIndex]) return commitTypes[selectedIndex];
...
async function main() {
  ...
  try {
    ...
    const args = ['commit', '-m', `${type}: ${subject}`];
